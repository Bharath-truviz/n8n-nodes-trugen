	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			...
			if (operation === 'createAgent') {
				...
				let llmModel = '';
				if (this.getNodeParameter('llmProvider', i) === 'openai') {
					llmModel = this.getNodeParameter('llmModel_openai', i) as string;
				} else if (this.getNodeParameter('llmProvider', i) === 'groq') {
					...
					avatars: [
						{
							...
							config: {
								llm: {
									provider: this.getNodeParameter('llmProvider', i),
									model: llmModel,
								},
								stt: {
			...
			// Get Conversation by ID Block
			if (operation === 'getConversation') {
				const conversationId = this.getNodeParameter('conversation_id', i);
				const wait = this.getNodeParameter('wait_for_completion', i, false);
				...
				if (wait) {
					let retries = 0;
					while (response.status !== 'Completed' && retries < 5) {
